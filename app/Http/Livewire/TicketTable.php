...
class TicketTable extends DataTableComponent
{
        ...
        return [
            Column::make('Actions')
                ->label(function ($row, Column $column) {
                    $delete_route = route('admin.tickets.destroy', $row->id);
                    $edit_route = route('admin.tickets.edit', $row->id);
            ...
            Column::make('status')
                ->format(function ($value, $data, Column $column) {
                    $route = route('admin.tickets.status');
    ...
    public function filters(): array
    {
        return [
            ...
            //     ])
            //     ->filter(function (Builder $builder, string $value) {
            //         $builder->where('tickets.name', 'like', '%' . $value . '%');
            //     }),
        ];
    }
    ...
    {
        $modelData = new Ticket;
        return Excel::download(new CustomExport($this->getSelected(), $modelData), 'tickets.xlsx');
    }
}
