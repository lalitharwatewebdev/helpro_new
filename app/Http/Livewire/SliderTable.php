...
class SliderTable extends DataTableComponent
{
        ...
        return [
            ...
            Column::make('Actions')
                ->label(function ($row, Column $column) {
                    $delete_route = route('admin.sliders.destroy', $row->id);
                    $edit_route = route('admin.sliders.edit', $row->id);
    ...
    public function filters(): array
    {
        return [
            ...
            //     ])
            //     ->filter(function (Builder $builder, string $value) {
            //         $builder->where('sliders.name', 'like', '%' . $value . '%');
            //     }),
        ];
    }
    ...
    {
        $modelData = new Slider;
        return Excel::download(new CustomExport($this->getSelected(), $modelData), 'sliders.xlsx');
    }
}
