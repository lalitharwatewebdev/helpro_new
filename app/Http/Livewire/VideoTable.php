...
class VideoTable extends DataTableComponent
{
        ...
        return [
            ...
            Column::make('status')
                ->format(function ($value, $data, Column $column) {
                    $route = route('admin.videos.status');
            ...
            Column::make('video')
                ->format(function ($row) {
                    if ($row) {
                        // return '<img src="' . asset($row) . '" class="view-on-click  rounded-circle">';
                        return '<iframe width="60" height="60"class="rounded-circle view-on-circle" src=" ' . $row .  '" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>';
                    }
                })
                ->html(),
            ...
            Column::make('Actions')
                ->label(function ($row, Column $column) {
                    $delete_route = route('admin.videos.destroy', $row->id);
                    $edit_route = route('admin.videos.edit', $row->id);
    ...
    public function filters(): array
    {
        return [
            ...
            //     ])
            //     ->filter(function (Builder $builder, string $value) {
            //         $builder->where('videos.name', 'like', '%' . $value . '%');
            //     }),
        ];
    }
    ...
    {
        $modelData = new Video;
        return Excel::download(new CustomExport($this->getSelected(), $modelData), 'videos.xlsx');
    }
}
